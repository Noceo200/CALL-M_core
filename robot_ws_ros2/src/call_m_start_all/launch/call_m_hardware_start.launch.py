...
def run_launch_with_parameters(context):
    ...
    namespace = 'callm'+context.launch_configurations['robot_id']
    ...
    if(not check_args(namespace,version,mode,nav_type,nav_mode,map_loc,slam_param)):
        return
    ...
    cmd = ['ros2', 'launch']
    suffix = ['use_sim_time:=false','namespace:='+namespace]
...
def check_args(namespace,version,mode,nav_type,nav_mode,map_loc,slam_param):
    tuto = """
    call_m_start_all package launched (hardware version)
    POSSIBLE PARAMETERS: 
    robot_id: Any values, (namespace will be 'callm<robot_id>')
