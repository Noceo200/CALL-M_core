...
def run_launch_with_parameters(context):
    ...
    if(not check_args(namespace,mode,nav_type,nav_mode,map_loc,slam_param)):
        return
    ...
    cmd = ['ros2', 'launch']
    suffix = ['use_sim_time:=true','namespace:='+namespace]
...
def check_args(namespace,mode,nav_type,nav_mode,map_loc,slam_param):
    tuto = """
